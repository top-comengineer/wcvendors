...
<p><?php printf( __( 'You have received an order from %s. Their order is as follows:', 'wcvendors' ), $order->get_billing_first_name() . ' ' . $order->get_billing_last_name() ); ?></p>

<?php do_action( 'woocommerce_email_before_order_table', $order, true ); ?>

<h2><?php printf( __( 'Order: %s', 'wcvendors'), $order->get_order_number() ); ?> (<?php printf( '<time datetime="%s">%s</time>', date_i18n( 'c', strtotime( $order->get_date_created() ) ), date_i18n( woocommerce_date_format(), strtotime( $order->get_date_created() ) ) ); ?>)</h2>
